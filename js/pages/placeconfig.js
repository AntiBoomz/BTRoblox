...
pageInit.placeconfig = placeId => {
	if(SETTINGS.get("placeConfigure.versionHistory")) {
		...
		async function loadPage(page) {
			...
			for(let i = innerPageStart; i <= innerPageEnd; i++) {
				const url = `https://www.roblox.com/places/version-history-items?assetID=${placeId}&page=${i}`
	
				innerPromises.push(fetch(url).then(async res => {
					const text = await res.text()
					const matches = text.match(/(<tr>[^]*?<\/tr>)/gm) || []
					
					return matches.filter(x => x.includes("asset-version-id=")).map(x => {
						return {
							versionNumber: x.match(/<td>(\d+)<\/td>/)[1],
							created: x.match(/<td>(\d*\/\d*\/\d*[^<]+)<\/td>/)[1],
		...
		document.documentElement
			.$on("click", ".btr-version-revert", e => {
				...
				RobloxApi.www.revertPlaceToVersion(versionId).then(success => {
					isBusy = false
					if(success) { loadPage(1) }
				})
			})
			.$on("click", ".btr-version-download", e => {
				...
				const fileName = `${placeName}-${version}.rbxl`
		...
		const initClick = ev => {
			if(ev.target.matches("#versionHistoryTab, #versionHistoryTab *")) {
				document.$off("click", initClick)
				loadPage(1)
			}
		}
